...
- 🎯 Entry points to execute your program using `python -m <test_fast_api>` or `$ test_fast_api` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/kapoc3/test-fast-api/branch/main/graph/badge.svg?token=test-fast-api_token_here)](https://codecov.io/gh/kapoc3/test-fast-api)
[![CI](https://github.com/kapoc3/test-fast-api/actions/workflows/main.yml/badge.svg)](https://github.com/kapoc3/test-fast-api/actions/workflows/main.yml)

Awesome test_fast_api created by kapoc3

## Install

from source
```bash
git clone https://github.com/kapoc3/test-fast-api test_fast_api
cd test_fast_api
make install
```
...
```bash
pip install test_fast_api
```

## Executing

```bash
$ test_fast_api run --port 8080
```

or

```bash
python -m test_fast_api run --port 8080
```

or

```bash
$ uvicorn test_fast_api:app
```
...
```bash
❯ test_fast_api --help
Usage: test_fast_api [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ test_fast_api create-user --help
Usage: test_fast_api create-user [OPTIONS] USERNAME PASSWORD
...
```bash
test_fast_api create-user admin admin --superuser
```
...
Run with `test_fast_api run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `test_fast_api create-user`.
...
-----------------------------------------------------
test_fast_api/__init__.py              4      0   100%
test_fast_api/app.py                  16      1    94%
test_fast_api/cli.py                  21      0   100%
test_fast_api/config.py                5      0   100%
test_fast_api/db.py                   10      0   100%
test_fast_api/models/__init__.py       0      0   100%
test_fast_api/models/content.py       47      1    98%
test_fast_api/routes/__init__.py      11      0   100%
test_fast_api/routes/content.py       52     25    52%
test_fast_api/routes/security.py      15      1    93%
test_fast_api/routes/user.py          52     26    50%
test_fast_api/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from test_fast_api.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in test_fast_api/default.toml.

### As environment variables
```bash
export test_fast_api_KEY=value
export test_fast_api_KEY="@int 42"
export test_fast_api_KEY="@jinja {{ this.db.uri }}"
export test_fast_api_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
test_fast_api_ENV=production test_fast_api run
```
